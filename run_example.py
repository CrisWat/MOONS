...
if ((cat.CENTRE_CUT[0] == 0.0) & (cat.CENTRE_CUT[1] == 0.0)):
    NSC_radius, l_centre, b_centre = 0.07, 0, 0

    A0 = np.where((rmag_cut!=-999) & (rmag_cut<14) )
    ...
    b = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T0_RC_NSC, 2, 'T')
    c = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T0_RGB_NSC, 3, 'T')
    d = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T1_RC_NSC, 2, 'T')
    e = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T1_RGB_NSC, 3, 'T')
    d = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T2_RC, 4, 'T')
    e = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T2_RGB, 5, 'T')
    f = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T3_RC, 6, 'T')
    g = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T3_RGB, 7, 'T')
    h = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T4_RC_NSC, 6, 'T')
    i = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T4_RGB_NSC, 7, 'T')
    l = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T5_RC, 8, 'T')
    m = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T5_RGB, 9, 'T')
    ...
    file=open('./'+name_output[:-6]+'.type', "w")
    file.write("# ID          RA        DEC        flag   priority   mag  \n".format())
    for val in zip(a[0], a[1], a[2], a[4], a[5]):
        file.write('  {:10}   {:=9.5f}   {:=9.5f}   A     {:=1.0f}   {:=7.4f}   \n'.format(val[0],val[1],val[2],val[3],val[4]))
    for val in zip(b[0], b[1], b[2], b[4], b[5]):
        file.write('  {:10}   {:=9.5f}   {:=9.5f}   RC    {:=1.0f}   {:=7.4f}   \n'.format(val[0],val[1],val[2],val[3],val[4]))
    for val in zip(c[0], c[1], c[2], c[4], c[5]):
        file.write('  {:10}   {:=9.5f}   {:=9.5f}   RGB   {:=1.0f}   {:=7.4f}   \n'.format(val[0],val[1],val[2],val[3],val[4]))
    for val in zip(idd_sky, ra_sky, ra_dec, priority_sky, mag_sky):
        file.write('  {:10}   {:=9.5f}   {:=9.5f}   Sky   {:=1.0f}   {:=7.4f}   \n'.format(val[0],val[1],val[2],val[3],val[4]))
    for val in zip(d[0], d[1], d[2], d[4], d[5]):
        file.write('  {:10}   {:=9.5f}   {:=9.5f}   RC    {:=1.0f}   {:=7.4f}   \n'.format(val[0],val[1],val[2],val[3],val[4]))
    for val in zip(e[0], e[1], e[2], e[4], e[5]):
        file.write('  {:10}   {:=9.5f}   {:=9.5f}   RGB   {:=1.0f}   {:=7.4f}   \n'.format(val[0],val[1],val[2],val[3],val[4]))
    ...
    A0 = np.where((rmag_cut!=-999) & (rmag_cut<15) )

    T0_RC = np.where( (blend_RC=='a0'))
    T0_RGB = np.where( (blend_RGB=='a0'))

    T1_RC = np.where( (blend_RC=='a1'))
    T1_RGB = np.where( (blend_RGB=='a1'))

    T2_RC = np.where( (blend_RC=='a2'))
    T2_RGB = np.where( (blend_RGB=='a2'))


    a = cat.make_priority_and_flag(ID_cut, ra_cut, dec_cut, rmag_cut, A0, 0, 'A')
    b = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T0_RC, 1, 'T')
    c = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T0_RGB, 1, 'T')
    d = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T1_RC, 2, 'T')
    e = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T1_RGB, 2, 'T')
    f = cat.make_priority_and_flag(ID_RC, ra_RC, dec_RC, Jmag_RC, T2_RC, 3, 'T')
    g = cat.make_priority_and_flag(ID_RGB, ra_RGB, dec_RGB, Jmag_RGB, T2_RGB, 3, 'T')
...
HessCMD.plotHess(K0_all, JK0_all,K0_RC,JK0_RC,K0_RGB,JK0_RGB, levels=np.arange(20,300,10), cbarrtitle='Number', xlab='$J_{0}$ $-$ $K_{0} (mag)$', ylab='$Ks_{0} (mag)$', saveas=name_output+'.png', cbarr='Yes',xlims=[-1,5], ylims=[15,5],colormap='jet',ftitle = name_output+".pdf")
